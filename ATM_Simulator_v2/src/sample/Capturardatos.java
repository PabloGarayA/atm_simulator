...
public class Capturardatos implements Initializable {
    ...
    @FXML
    private void  regresarADepositoEnEfectivo(ActionEvent event) {

        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("depositoenefectivo.fxml"));
            AnchorPane pane = loader.load();

            Depositoenefectivo regresarADepositoEnEfectivo = loader.getController();
            regresarADepositoEnEfectivo.recogerNumeroDeC(displayNumeroDeCuenta.getText());
            pantallaCapturarDatos.getChildren().setAll(pane);
    ...
    @FXML
    private void continuarARecibo(ActionEvent event) {

        try{
            FXMLLoader loader = new FXMLLoader(getClass().getResource("recibo.fxml"));
            AnchorPane pane = loader.load();

            Recibo continuarARecibo = loader.getController();
            continuarARecibo.recogerImpo(displayImporte.getText());
            continuarARecibo.recogerNumeroDeC(displayNumeroDeCuenta.getText());
            continuarARecibo.recoger(displayMotivo.getText());
            pantallaCapturarDatos.getChildren().setAll(pane);
